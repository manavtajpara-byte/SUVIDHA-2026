...
export default function BusPassPage() {
    ...
    return (
        <div style={styles.container}>
            ...
            <div style={styles.card}>
                {step === 1 && (
                    <form onSubmit={handleApply} style={styles.form}>
                        <div style={styles.iconHeader}>
                            <Bus size={60} color="#9333ea" />
                            <p>Enter details for Monthly Bus Pass</p>
                        </div>
                        ...
                        <button type="submit" style={styles.submitBtn} disabled={!values.name || !values.mobile}>
                            Pay & Generate Pass (₹ 500)
                        </button>
                    </form>
                )}
                ...
                {step === 2 && (
                    <div style={styles.success}>
                        <CheckCircle2 size={120} color="#9333ea" />
                        <h3 style={styles.successTitle}>Pass Generated Successfully!</h3>
                        <p>Pass ID: BUS-2026-8821</p>
                        <div style={styles.receiptActions}>
                            <button style={styles.receiptBtn}>Download PDF</button>
            ...
            {isOpen && (
                <VirtualKeyboard onInput={handleInput} onDelete={handleDelete} onClose={closeKeyboard} />
            )}
            ...
            {showReceipt && (
                <Receipt
                    ...
                    amount={500}
                    customerName={values.name || 'Commuter'}
                    details={{
                        'Pass Type': 'Monthly All Route',
                        'Valid From': '18 Jan 2026',
                        'Valid To': '17 Feb 2026',
                        'Mobile': values.mobile || 'NA'
                    }}
...
const styles: Record<string, React.CSSProperties> = {
    ...
    title: { fontSize: '2.5rem', fontWeight: 900, margin: 0, color: '#9333ea' },
    ...
    submitBtn: { padding: '1.5rem', fontSize: '1.5rem', fontWeight: 'bold', backgroundColor: '#9333ea', color: 'white', border: 'none', borderRadius: '1rem', cursor: 'pointer' },
    success: { display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '1.5rem' },
    successTitle: { fontSize: '2.5rem', fontWeight: 900, color: '#9333ea', margin: 0 },
    receiptActions: { display: 'flex', gap: '1rem', marginTop: '1rem' },
    receiptBtn: { padding: '1rem 2rem', fontSize: '1.2rem', fontWeight: 'bold', border: '2px solid #9333ea', borderRadius: '1rem', background: 'none', color: '#9333ea', cursor: 'pointer' },
